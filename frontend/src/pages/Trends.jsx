...
export default function Trends() {
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto w-full">
      ...
      {/* THE SMART DOMAIN MODAL */}
      <AnimatePresence>
        {selectedDomain && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedDomain(null)} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40" />
            
            <motion.div 
              initial={{ opacity: 0, y: 100, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 100, scale: 0.95 }} 
              className="fixed top-[5%] left-0 right-0 md:left-1/2 md:-translate-x-1/2 md:w-full md:max-w-3xl h-[90vh] md:h-auto md:max-h-[90vh] bg-[#0A0A0A] border border-white/10 rounded-t-3xl md:rounded-3xl z-50 overflow-hidden flex flex-col shadow-2xl"
            >
              ...
              <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                ...
                {/* 🪄 Skills with Animated Save Buttons */}
                <div className="mb-10">
                  <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                    <FiTrendingUp /> Skills you must master
                  </h3>
                    ...
                    {selectedDomain.skills?.map((skill, idx) => (
                      <div key={idx} className="flex items-center gap-2 bg-[#121214] border border-white/5 px-3 py-2 rounded-xl shadow-sm">
                        ...
                        <button
                            ...
                            title={savedSkills.includes(skill) ? "Saved to Roadmap" : "Add to Roadmap"}
